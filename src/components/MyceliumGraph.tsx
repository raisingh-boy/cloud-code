...
function SomaticSphere({
  node, isSelected, isHovered, isActiveAudio, color, onClick, onHover, currentWorld, overlayUser
}: {
  node: SomaticNode; isSelected: boolean; isHovered: boolean; isActiveAudio: boolean;
  color: string; onClick: (n: SomaticNode) => void; onHover: (id: string | null) => void;
  currentWorld: World; overlayUser: string | null;
}) {
...
// ============================================================
function NodeLabel({ node, language, SCALE, isSelected, isActiveAudio, isHovered }: {
  node: SomaticNode; language: 'ru' | 'en'; SCALE: number;
  isSelected: boolean; isActiveAudio: boolean; isHovered: boolean;
}) {
  const showLabel = isSelected || isActiveAudio || isHovered || node.status !== 'seed';
...
function GraphScene({
  ...
  useEffect(() => {
    ...
    const newNodes = nodes.map((n, idx) => {
      ...
      // Deterministic hash from node id for stable positions across re-renders
      const idHash = n.id.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);

      // Calculate target positions dynamically for this currentWorld
      let tx = 0, ty = 0, tz = 0;
      if (currentWorld === 'atlas') {
        const domainOrder = ['body', 'science', 'philosophy', 'movement', 'cognition', 'hybrid'];
        const domainIdx = domainOrder.indexOf(n.domain);
        const sectorAngle = Math.PI * 2 / 6; // 60° per domain
        // Rank within domain for deterministic angular spread and ring assignment
        const domainNodes = nodes.filter(fn => fn.domain === n.domain);
        const domainRank = domainNodes.findIndex(fn => fn.id === n.id);
        const domainSize = Math.max(domainNodes.length, 1);
        const normalizedRank = domainSize > 1 ? domainRank / (domainSize - 1) : 0.5;
        // ±70% of sector width (±21° out of 30° half-sector)
        const spread = (normalizedRank - 0.5) * sectorAngle * 1.4;
        const angle = domainIdx * sectorAngle + spread;
        // 5 rings: 100, 155, 210, 265, 320 physics units
        const ringIdx = domainRank % 5;
        const dist = 100 + ringIdx * 55;
        tx = Math.cos(angle) * dist;
        ty = Math.sin(angle) * dist;
        tz = ((domainRank % 4) - 1.5) * 40; // -60, -20, +20, +60
      } else if (currentWorld === 'field') {
        // Deterministic golden-angle spiral, scaled by node count
        const fieldNodeCount = nodes.filter(fn => fn.world === 'field').length;
        const densityScale = Math.min(Math.max(Math.sqrt(fieldNodeCount / 16), 0.3), 1.8);
        const goldenAngle = idx * 2.399963; // golden ratio * 2π
        const dist = (60 + (idHash % 80)) * densityScale;
        tx = Math.cos(goldenAngle) * dist;
        ty = Math.sin(goldenAngle) * dist;
        tz = ((idHash % 7) - 3) * 20 * densityScale;
      } else {
        // Personal Universe view
        if (n.id === 'central-me') {
          tx = 0; ty = 0; tz = 0;
        } else {
          // Deterministic orbit around central-me
          const meAngle = idx * 2.399963; // golden angle
          const dist = 90 + (idHash % 80);
          tx = Math.cos(meAngle) * dist;
          ty = Math.sin(meAngle) * dist;
          tz = ((idx % 5) - 2) * 28;
        }
      }
  ...
  useFrame((state) => {
    ...
    // Scale repulsion for field world so sparse graphs don't scatter to infinity
    const fieldCount = currentWorld === 'field'
      ? gState.nodes.filter(n => n.world === 'field').length
      : 0;
    const repulsionStrength = currentWorld === 'field'
      ? 2200 * Math.min(1.0, fieldCount / 12)
      : 2200;
